...
export function SushiSection({ projectId }: { projectId: string }) {
      ...
      return (
         <div className="flex flex-col h-full overflow-hidden">
            <div className="flex flex-1">
               ...
               <div className="flex-1 p-12 flex flex-col justify-center gap-6">
                  ...
                  <div className="flex flex-col gap-4 mt-4">
                     <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-gray-900">
                           <Check className="w-5 h-5" />
                        </div>
                        <span className="text-gray-900">Auto-generated images and copy</span>
                     </div>
                     <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-gray-900">
                           <Check className="w-5 h-5" />
                        </div>
                        <span className="text-gray-900">Multi-platform specific formats</span>
                     </div>
                     <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-gray-900">
                           <Check className="w-5 h-5" />
                        </div>
                        <span className="text-gray-900">AI-driven marketing strategy</span>
                     </div>
                  </div>
                  <Button
                     onClick={handleStart}
               ...
               <div className="flex-1 relative overflow-hidden bg-[#e7e5df] rounded-4xl">
                  <div className="absolute inset-0 opacity-20 pointer-events-none">
                     <div className="absolute top-[-10%] right-[-10%] w-64 h-64 bg-white rounded-full blur-3xl animate-pulse" />
                     <div className="absolute bottom-[-10%] left-[-10%] w-96 h-96 bg-blue-400 rounded-full blur-3xl animate-pulse" />
                  </div>
                  <div className="h-full flex items-center justify-center p-12">
                     <motion.div
                        initial={{ y: 50, opacity: 0 }}
                        animate={{ y: 0, opacity: 1 }}
                        className="relative z-10 w-full max-w-sm aspect-[9/16] bg-white rounded-[40px] shadow-2xl border-8 border-slate-900 overflow-hidden"
                     >
                        ...
                        <div className="flex-1 overflow-y-auto pb-4">
                           <div className="p-4">
                              <div className="w-full aspect-square bg-slate-100 rounded-xl mb-4 overflow-hidden">
                                 <div className="w-full h-full bg-gradient-to-tr from-orange-400 to-pink-500 animate-pulse" />
                              </div>
                        ...
                        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 w-12 h-1 bg-slate-200 rounded-full" />
                     </motion.div>
                  </div>
               </div>
            </div>
            ...
            <div className="max-w-3xl mx-auto w-full py-12 px-6">
               <div className="flex justify-between items-center">
                  <div className="flex items-center gap-4">
                     <div className="space-y-1">
                        <h3 className="text-gray-900">Step {currentStep + 1} of 3</h3>
                        {/* <div className="flex items-center gap-1">
                           {[0, 1, 2].map((i) => (
                              <div key={i} className={cn("h-1.5 rounded-full transition-all duration-300", i <= currentStep ? "w-8 bg-[#0099ff]" : "w-4 bg-slate-200")} />
                           ))}
                        </div> */}
                     </div>
                  </div>
               ...
               <AnimatePresence mode="wait">
                  <motion.div
                     ...
                     exit={{ opacity: 0, x: -20 }}
                  >
                     <h2 className="text-md text-gray-900 mb-3">{question.question}</h2>
                     <div className="grid grid-cols-1 gap-1">
                        {question.options?.map((option, idx) => (
                           <button
                              ...
                              className={cn(
                                 "flex flex-col items-start p-4 rounded-sm border text-left",
                                 question.answer === option.title
                                    ? "border-[#0099ff]"
                                    : "border bg-white hover:border-slate-200"
                              )}
                           >
                              <div className="flex items-center justify-between w-full">
                                 <span className="font-bold text-sm text-slate-900">{option.title}</span>
                                 <div className={cn(
                                    "w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors",
                                    question.answer === option.title ? "bg-[#0099ff] border-[#0099ff]" : "border-slate-200"
                                 )}>
                                    {question.answer === option.title && <Check className="w-4 h-4 text-white" />}
                                 </div>
                              </div>
                              <p className="text-slate-500 text-xs">{option.subtitle}</p>
                           </button>
                        ))}
                     </div>
                     ...
                     <div className="flex justify-end mt-1 gap-1">
                        {currentStep > 0 && (
                           <Button variant="outline" className="rounded-sm w-fit h-7 bg-white border text-black hover:bg-white font-normal" onClick={() => setCurrentStep(prev => prev - 1)}>
                              Back
                           </Button>
                        )}
                        {question.answer && (
   ...
   return (
      <div className="flex flex-col h-full w-full overflow-y-auto">
         <div className="sticky top-0 z-20 bg-white/80 backdrop-blur-md px-8 py-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
               <div>
                  <TextShimmer className="">Your Social Content</TextShimmer>
                  <p className="text-gray-500 text-xs">{data.posts?.length} posts generated</p>
                  <p className="text-gray-500 text-xs">Creation img coming soon...</p>
               </div>
            </div>
            <div className="flex items-center gap-2">
               <Button variant="outline" size="sm" onClick={() => setData({ ...data, status: "idle" })} className="rounded-sm w-fit h-7 bg-white border text-black hover:bg-white font-normal">
         ...
         <div className="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
            {(data.posts || []).length > 0 ? (data.posts || []).map((post) => (
               <div key={post.id} className="bg-white rounded-sm shadow-xs border border-slate-200 overflow-hidden flex flex-col group hover:shadow-xl transition-all duration-300">
                  <div className="px-6 py-4 border-b border-slate-50 flex items-center justify-between">
                     <div className="flex items-center gap-2">
                        ...
                        {post.id.includes("tiktok") && <Smartphone className="w-5 h-5 text-black" />}
                        <span className="font-bold text-sm uppercase tracking-wider text-slate-700">{post.platform}</span>
                     </div>
                     <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button className="p-2 hover:bg-slate-100 rounded-lg text-slate-500" onClick={() => setEditModal({ open: true, post })}>
                  ...
                  {/* <div className="relative aspect-square overflow-hidden bg-slate-900">
                     <img src={post.imageUrl} alt="Generated post" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
                  ...
                  <div className="p-6 flex-1 flex flex-col">
                     <div className="flex-1 rounded-sm p-4 mb-6 border text-slate-700 text-sm leading-relaxed whitespace-pre-wrap italic">
                        "{post.content}"
                     </div>
                     ...
                     <div className="flex gap-2">
                        <Button
                           variant="outline"
                           className="rounded-sm w-fit h-7 bg-white border text-black hover:bg-white font-normal"
                           onClick={() => {
               ...
               <div className="col-span-full py-20 flex flex-col items-center justify-center text-center space-y-6">
                  ...
                  <div className="space-y-2">
                     <h3 className="text-xl font-bold text-slate-900">No posts generated yet</h3>
                     <p className="text-slate-500 max-w-sm mx-auto">The AI strategist didn't output any content. Try regenerating the pack with a different tone.</p>
                  </div>
            ...
            {editModal.open && editModal.post && (
               <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                  ...
                  <motion.div
                     ...
                     className="relative w-full max-w-4xl bg-white rounded-3xl overflow-hidden shadow-2xl flex flex-col md:flex-row h-full max-h-[80vh]"
                  >
                     ...
                     <div className="w-full md:w-[400px] p-10 flex flex-col gap-8 bg-white overflow-y-auto">
                        <div className="flex items-center justify-between">
                           <h2 className="text-2xl font-bold text-slate-900 italic uppercase">Social Content Edit</h2>
                           <button onClick={() => setEditModal({ open: false, post: null })} className="p-2 hover:bg-slate-100 rounded-full">
                              <Trash2 className="w-5 h-5 text-slate-400" />
                           </button>
                        </div>

                        <div className="space-y-4">
                           <label className="text-sm font-bold text-slate-400 uppercase tracking-widest">Platform</label>
                           <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                              ...
                              {editModal.post.platform === "TikTok" && <Smartphone className="w-6 h-6 text-black" />}
                              <span className="font-bold text-slate-900">{editModal.post.platform}</span>
                           </div>
                        </div>

                        <div className="space-y-4 flex-1">
                           <label className="text-sm font-bold text-slate-400 uppercase tracking-widest">Post Content</label>
                           <textarea
                              className="w-full h-48 bg-slate-50 border border-slate-100 rounded-2xl p-6 text-slate-700 text-lg leading-relaxed focus:bg-white focus:border-[#0099ff] transition-all outline-none resize-none"
                        ...
                        <div className="pt-6 border-t border-slate-100 flex gap-4">
                           <Button variant="outline" className="flex-1 rounded-2xl h-14" onClick={() => setEditModal({ open: false, post: null })}>Cancel</Button>
